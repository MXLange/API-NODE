...
export class UfDAO {
  ...
  async criar({ sigla, nome, status }: ICadastrarUf): Promise<any> {
    ...
    if (jaExisteSigla) {
      throw new AppError(`Já existe uma UF com a sigla ${sigla}.`, 409);
    }
    ...
    if (jaExisteNome) {
      throw new AppError(`Já existe uma UF com o nome ${nome}.`, 409);
    }
    ...
    if (!resultado) {
      throw new AppError("Não foi possível incluir UF no banco de dados.", 400, queryRunner);
    }
    ...
    if (!retorno) {
      throw new AppError("Não foi possível gerar o retorno, porém seu cadastro foi concluído.");
    }
  ...
  async alterar({ codigoUF, sigla, nome, status }: IAlterarUf) {
    ...
    if (jaExisteSigla && jaExisteSigla.codigoUF !== codigoUF) {
      throw new AppError(`Já existe uma UF com a sigla ${sigla}.`);
    }
    ...
    if (jaExisteNome && jaExisteNome.codigoUF !== codigoUF) {
      throw new AppError(`Já existe uma UF com o nome ${nome}.`);
    }
