...
export class MunicipioDAO {
  ...
  async criar({ codigoUF, nome, status }: ICadastrarMunicipio): Promise<any> {
    ...
    if (jaExiste.length !== 0) throw new AppError(`Já existe uma município com o nome ${nome} para esta UF.`, 400, queryRunner)
  ...
  async alterar({ codigoMunicipio, codigoUF, nome, status }: IAlterarMunicipio): Promise<Array<any>> {
    ...
    if (jaExiste.length !== 0 && jaExiste[0].CODIGO_MUNICIPIO !== codigoMunicipio) throw new AppError(`Já existe uma município com o nome ${nome} para esta UF.`, 400, queryRunner)
    ...
    if (!municipio) throw new AppError("Insira um código de município válido.", 400, queryRunner)
    ...
    if (!existeUf) throw new AppError("Por favor, insira um código de UF válido", 400, queryRunner)
  ...
  async deletar(codigoMunicipio: number) {
    ...
    if (!existeMunicipio) throw new AppError("Insira um código de município válido.")
