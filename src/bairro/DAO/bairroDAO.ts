...
export class BairroDAO {
  ...
  async criar({ codigoMunicipio, nome, status }: ICadastrarBairro): Promise<any> {
    ...
    if (jaExiste.length !== 0) throw new AppError(`Já existe um bairro com o nome ${nome} para este município.`, 400, queryRunner)
  ...
  async alterar({ codigoBairro, codigoMunicipio, nome, status }: IAlterarBairro): Promise<Array<any>> {
    ...
    if (jaExiste.length !== 0 && jaExiste[0].CODIGO_BAIRRO !== codigoBairro) throw new AppError(`Já existe um bairro com o nome ${nome} para este município.`)
  ...
  async deletar(codigoBairro: number) {
    ...
    if (!existeBairro) throw new AppError("Insira um código de município válido.")
